...
public class PaperServiceImpl extends ServiceImpl<PaperMapper, Paper> implements PaperImplService {
  ...
  @Autowired
    ...
    public CommonResult getallpaper() {
        QueryWrapper<Paper> queryWrapper=new QueryWrapper();
        return CommonResult.success(baseMapper.selectList(queryWrapper),"查看所有试卷成功");
    }
...
/*                QueryWrapper<Listen> queryWrapper1=new QueryWrapper<>();
     ...
     */
    @Override
    public CommonResult randomPaper(Map map) {
              ...
              //  System.out.println("------------" + a.size());
                ...
                for (int j = 0; j < mylist.size(); j++) {
                   // System.out.println(mylist.get(j));
                    ...
                    Recording recording = new Recording();
                    recording.setExId((Integer) mylist.get(j)+1);
                    recording.setType(type);
                    recording.setPaperId(paper.getId());
                    recording.setState(1);
                    recording.setExLevel(ex_level);
   ...
   /*                 QueryWrapper<Listen> queryWrapper2=new QueryWrapper<>();
        ...
        return CommonResult.success(paper.getId(),"成功");
    }
    ...
    public CommonResult createExam(Map map) {
       ...
       // paper.setScore(Integer.parseInt((String)map.get("score")));
        ...
        ZoneId zoneId = ZoneId.systemDefault();
          ...
          Date b=format.parse((String) map.get("end_time"));
          ...
          //统计考试班级
        List list1= (List) map.get("ks_class");
        List list2= (List) map.get("ks_tea");
        System.out.println(list1);
        for (int i = 0; i < list1.size(); i++) {
            Object lo = list1.get(i);
            Map entry = (Map) lo;
            String  specialized= (String) entry.get("specialized");
            String  sch_paper= (String) entry.get("sch_paper");

            ClassPaper classPaper = new ClassPaper();
            classPaper.setPaperId(paper_id);
            classPaper.setSpecialized(specialized);
            classPaper.setSch_paper(specialized);
             classPaperMapper.insert(classPaper);
        }
        for (int i = 0; i < list2.size(); i++) {
            Object lo = list2.get(i);
            Map entry = (Map) lo;
            String  specialized= (String) entry.get("tea_name");
            String  sch_paper= (String) entry.get("tea_school");
            ExamTeacher examTeacher=new ExamTeacher();
                examTeacher.setExamTeacher(specialized);
                examTeacher.setTeacherSchool(sch_paper);
                examTeacher.setExamId(paper_id);
               examTeacher.setTeacherState("未批阅");
        ...
        return CommonResult.failed("创建考试失败,试卷库中没这个试卷");
    }
